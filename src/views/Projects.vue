...
const getIndex = (p: Project): number =>
  galleryIndex.value[p.id] ?? 0;
...
<template>
  <section class="py-24 px-5 max-w-7xl mx-auto text-center">
    <h1 class="text-4xl md:text-6xl font-extrabold text-green-500 mb-10">
      Proyectos
    </h1>
    ...
    <div v-if="projects.length" class="grid md:grid-cols-2 xl:grid-cols-3 gap-10">
      <div
        ...
        :key="project.id"
        class="bg-gray-800 rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-all duration-300"
      >
        <img
          v-if="project.image"
          :src="safeFileUrl(project.image)"
        ...
        <div class="p-6 text-left">
          ...
          <a
            ...
            :href="project.link"
            target="_blank"
            class="inline-block px-4 py-2 text-green-500 border border-green-500 rounded-lg hover:bg-green-500 hover:text-black transition-all"
          >
            Ver Proyecto
          </a>
            ...
            <div v-else class="relative h-32 overflow-hidden rounded-md">
              <transition name="fade" mode="out-in">
                <img
                  ...
                  @click="
                    zoomImage = safeFileUrl(project.gallery[getIndex(project)])
                  "
                />
              </transition>
            </div>
    ...
    >
      <img
        :src="zoomImage"
        class="max-w-4xl max-h-[90vh] rounded-lg shadow-2xl border border-gray-700"
      />
    </div>
  </section>
</template>
